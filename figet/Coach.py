...
class Coach(object):
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            train_model_loss = self.train_proj_epoch(epoch)
        ...
        for epoch in range(1, self.args.epochs + 1):
            train_classif_loss = self.train_classif_epoch(epoch)

            log.info(f"Results epoch {epoch}: Classif:{train_classif_loss:.5f}")
    ...
    def train_proj_epoch(self, epoch):
        """:param epoch: int >= 1"""
    ...
    def train_classif_epoch(self, epoch):
        """:param epoch: int >= 1"""
        if self.args.extra_shuffle == 1:
            self.train_data.shuffle()
        ...
        #           f"cos_fact:{self.args.cosine_factor}, norm_fact:{self.args.norm_factor}, "
        #           f"max norm:{all_pred_norm.max().item()}, min norm:{all_pred_norm.min().item()}")
        return np.mean(total_classif_loss)
